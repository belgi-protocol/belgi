...
| Builtin pack manifest is self-consistent | `belgi pack verify --in belgi/_protocol_packs/v1` | Exit `0` = PASS; exit `10` = NO-GO; exit `20` = USER_ERROR; exit `30` = INTERNAL_ERROR |
| Installed pack (wheel) is self-consistent | `belgi pack verify --builtin` | Exit `0` = PASS; exit `10` = NO-GO; exit `20` = USER_ERROR; exit `30` = INTERNAL_ERROR |
...
- `tools/sweep.py` is the CLI shell entrypoint only; fixed input-family law lives in `tools/_sweep/input_surface_spec.py`, managed-surface family law lives in `tools/_sweep/managed_surface_spec.py`, resolution/composition live in `tools/_sweep/managed_surfaces.py` and `tools/_sweep/inputs.py`, and invariant registration lives under `tools/_sweep/`.
...
1) Add the new `- invariant_id:` entry to `docs/operations/consistency-sweep.md`.
2) Implement the corresponding check in the correct owner family under `tools/_sweep/invariants/*.py`.
3) Register the invariant in `tools/_sweep/registry.py` to preserve the explicit ordered catalog.
4) Update `tools/_sweep/input_surface_spec.py` if a fixed explicit input family changes, update `tools/_sweep/managed_surface_spec.py` if the managed operational surface family law changes, and update `tools/_sweep/inputs.py` / `tools/_sweep/managed_surfaces.py` only when the composition or resolver logic itself changes.
5) If you edited `docs/operations/consistency-sweep.md`, run `python -m tools.build_builtin_pack` so the canonical mirror is regenerated by the owning tool.
6) Run `python -m tools.sweep consistency --repo .` and confirm exit `0` (or expected FAIL if you are mid-migration).
...
Exit codes are standardized; SSOT: `docs/operations/exit-codes.md`.
...
- `20`: USER_ERROR (usage/input contract issues, directory not found, etc.)
- `30`: INTERNAL_ERROR (unexpected internal failure)

Verification checks:
- All files in manifest exist with matching SHA-256 and size.

Shared owner note:
- `belgi/protocol/pack_surface_inventory.py` owns the builtin pack manifest name, content prefixes, folder/file allowlists, and C3 canonical-mirror bindings reused by `belgi.protocol.pack`, `tools.build_builtin_pack`, `tools.check_drift`, and the sweep canonicals invariant.
...
- `20`: USER_ERROR (usage/input contract issues, missing --demo, directory not found, etc.)
- `30`: INTERNAL_ERROR (unexpected internal failure)
