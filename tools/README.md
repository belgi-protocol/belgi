...
2) Implement the corresponding check in the correct owner family under `tools/consistency/invariants/*.py`.
3) Register the invariant in `tools/consistency/registry.py` to preserve the explicit ordered catalog.
4) Update `tools/consistency/inputs.py` if the new owner file or managed surface changes governed sweep input authority.
