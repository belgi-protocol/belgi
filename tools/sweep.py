...
def _extract_spec_invariant_ids(repo_root: Path) -> list[str]:
    ...
    spec_path = _common._resolve_repo_path(repo_root, CONSISTENCY_SPEC_DOC, must_exist=True, must_be_file=True)
    txt = _common.read_text(spec_path)
    ids = _SPEC_INVARIANT_ID_RE.findall(txt)
    if not ids:
        raise _common._UserInputError(f"no invariant_id entries found in {CONSISTENCY_SPEC_DOC}")
    ...
    if dups:
        raise _common._UserInputError(f"duplicate invariant_id entries in {CONSISTENCY_SPEC_DOC}: {sorted(dups)}")
...
def _git_tree_sha_excluding(
    ...
    *,
    ...
    return _common._git_tree_sha_excluding(repo_root, exclude_paths, blob_overrides=blob_overrides)
...
def _consistency_sweep_main(argv: list[str] | None = None) -> int:
    ...
    if not root.exists() or not root.is_dir():
        raise _common._UserInputError(f"repo root is not a directory: {root}")
    out_path = _common._resolve_repo_path(root, args.out, must_exist=False)
    return run_consistency_sweep(
        ...
        validate_repo_rel=_common._validate_repo_rel,
        extract_spec_invariant_ids=_extract_spec_invariant_ids,
        invariant_registry=_registry_owner.invariant_registry,
        canonical_inputs=_inputs_owner._canonical_inputs,
        repo_revision_getter=_git_tree_sha_excluding,
        resolve_existing_repo_file=lambda repo_root, rel: _common._resolve_repo_path(
            repo_root,
            rel,
            must_exist=True,
            must_be_file=True,
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        raise _common._UserInputError(f"Unknown command: {ns.cmd}")
