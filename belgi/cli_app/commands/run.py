...
def _render_adopter_readme(*, workspace_rel: str) -> str:
    ...
    toolchain_set_path = f"{workspace_rel}/runs/run-001/inputs/environment/toolchain-set.json"
    tolerances_path = f"{workspace_rel}/runs/run-001/inputs/environment/tolerances.json"
    return (
        "# BELGI Quickstart\n\n"
        ...
        "- ToolchainSet refs must point at actual repo-relative dependency/toolchain declaration surfaces in the evaluated revision truth envelope; local-only extras are rejected.\n"
        ...
        "- A Tolerances object must already exist in the evaluated revision truth envelope when explicitly referenced.\n"
...
def _render_runbook_template(*, run_id: str) -> str:
    ...
    return (
        ...
        "- do not mix `--toolchain-set-ref` with shorthand `--toolchain-ref` values.\n"
        "- use actual repo-relative dependency/toolchain declaration surfaces when R7 needs declared accounting context.\n"
        "- these refs must already exist in the evaluated revision truth envelope; local-only extras are rejected.\n"
        ...
        "- recommended object id: `tier.tolerances`.\n"
        "- explicit Tolerances refs must already exist in the evaluated revision truth envelope.\n"
