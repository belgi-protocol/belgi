...
def run(ctx: RCheckContext) -> list[CheckResult]:
    ...
    if not _command_ok(ctx, "supplychain-scan"):
        return [
            CheckResult(
                check_id="R7",
                status="FAIL",
                category="FR-COMMAND-FAILED",
                message="Required command missing/failed: belgi supplychain-scan.",
                pointers=[],
                remediation_next_instruction="Do ensure required command record belgi supplychain-scan exists with exit_code 0 in EvidenceManifest.commands_executed then re-run R.",
            )
        ]

    if not _has_policy_report_artifact(ctx, "policy.supplychain"):
        return [
            CheckResult(
                check_id="R7",
                status="FAIL",
                category="FR-SUPPLYCHAIN-SCAN-MISSING",
                message="Required policy_report artifact missing: id==policy.supplychain.",
                pointers=[],
                remediation_next_instruction="Do produce required evidence kind policy_report under the declared envelope then re-run R.",
            )
        ]

    payload, err = _load_policy_report(ctx, "policy.supplychain")
    if payload is None:
        return [
            CheckResult(
                check_id="R7",
    ...
    if not isinstance(failed, int) or isinstance(failed, bool):
        return [
            CheckResult(
                ...
                status="FAIL",
                category="FR-SCHEMA-ARTIFACT-INVALID",
                message="policy.supplychain summary.failed missing/invalid.",
                pointers=[],
                remediation_next_instruction="Do fix schema validation errors in required artifact then re-run R.",
            )
        ]
    ...
    return [
        CheckResult(
            ...
            status="PASS",
            category=None,
            message="R7 satisfied: belgi supplychain-scan recorded and policy.supplychain report indicates no failures.",
            pointers=[],
        )
    ]
