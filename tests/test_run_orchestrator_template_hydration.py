...
def test_orchestrate_rejects_noncanonical_toolchain_set_ref_before_c1(
    ...
    monkeypatch.setattr(
        ...
        lambda **_: pytest.fail("run_supplychain_scan should not execute after noncanonical ToolchainSet rejection"),
    )
...
def test_orchestrate_stages_run_local_tolerances_ref_before_c1(
    ...
    captured: dict[str, object] = {}
    source_repo = tmp_path / "src"
    source_tolerances_path = (
        source_repo / ".belgi" / "runs" / "run-001" / "inputs" / "environment" / "tolerances.json"
    )
